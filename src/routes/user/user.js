...
Router.post('/signup', async(req, res) => {
    try {
        ...
        if (!name || !password || !email) return res.status(400).json({ 'err': 'required filed must be posted' });
...
Router.post('/login', async(req, res) => {
    try {
        ...
        if (!password || !email) return res.status(400).json({ 'err': 'required filed not inserted' });
