import React, { useContext, useEffect, useState } from 'react';
import './style/Profile.css';
import constant from '../../const';
import M from 'materialize-css';
import { UserContext } from '../../App';

const Profile = () => {
  const [data, setData] = useState({ avatar: null, post: [] });
  const [image, setImage] = useState('');

  const { state, dispatch } = useContext(UserContext);
  useEffect(() => {
    async function fetchData() {
      try {
        const getData = await fetch(constant.localUrl + 'post/', {
          method: 'get',
          headers: {
            'Content-Type': 'application/json',
            Authorization: 'Bearer ' + JSON.parse(localStorage.getItem('jwt')),
          },
        });
        const result = await getData.json();
        if (result && result.posts) {
          console.log('init', result);
          await setData({ post: result.posts, avatar: result.avatar || null });
        }
      } catch (e) {
        console.error(e);
        M.toast({ html: 'something wrong', classes: '#c62828 red darken-3' });
      }
    }

    fetchData();
  }, []);
  const showInput = () => {
    const avator_selector = document.getElementById('avator-selector');
    avator_selector.click();
  };
  const uploadAvatar = async () => {
    try {
      console.log('image', image);
      if (image) {
        const formData = new FormData();
        formData.append('avatar', image); // appending file
        const sendAvatar = await fetch(constant.localUrl + 'user/', {
          method: 'put',
          redirect: 'follow',
          headers: {
            Authorization: 'Bearer ' + JSON.parse(localStorage.getItem('jwt')),
          },
          body: formData,
        });
        const result = await sendAvatar.json();
        if (sendAvatar.status.toString() === '400' || result.err)
          M.toast({
            html: result.err ? result.err : 'something wrong',
            classes: '#c62828 red darken-3',
          });
        else {
          console.log('avatar put', result);
          if (result.user.avatar)
            setData({ avatar: result.user.avatar, post: data.post });
        }
      }
    } catch (e) {
      console.error(e);
      M.toast({
        html: 'something wrong',
        classes: '#c62828 red darken-3',
      });
    }
  };
  return (
    <div className="container-user-page">
      {console.log('hiii', data)}
      <div className="df user-top-info">
        <div className="w40">
          <div className="avatar-wrapper">
            <img
              className="image-avatar avatar-profile"
              alt="user"
              src={
                data && data.avatar
                  ? data.avatar
                  : 'data:image/png;base64,'
              }
            />
            <div className="upload-avatar" onClick={showInput}>
              <input
                type="file"
                id="avator-selector"
                onChange={(e) => setImage(e.target.files[0])}
                onClick={uploadAvatar}
              ></input>
              <i className="material-icons">add_a_photo</i>
            </div>
          </div>
        </div>
        <div className="w40">
          <h4 style={{ color: 'grey' }}>
            {state && state.name ? state.name : 'loading'}
          </h4>
          <div className="df follow-info w70">
            <h5>85 posts</h5>
            <h5>0 followers</h5>
            <h5>100K following</h5>
          </div>
        </div>
      </div>

      {/*    gallery here*/}
      <div className="user-gallery">
        {data && data.post != {}
          ? data.post
              .map((item) => {
                return (
                  <img
                    className="gallery-item"
                    src={item.photo}
                    alt="gallery"
                  />
                );
              })
              .reverse()
          : null}
      </div>
    </div>
  );
};
export default Profile;
